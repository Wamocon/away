...
export default function AlertModal({
  ...
  const getIcon = () => {
    switch (type) {
      case 'danger': return <Trash2 className="text-rose-500" size={28} />;
      case 'success': return <CheckCircle2 className="text-emerald-500" size={28} />;
      case 'info': return <Info className="text-indigo-500" size={28} />;
      default: return <AlertTriangle className="text-amber-500" size={28} />;
    }
  };
  ...
  const getConfirmStyle = () => {
    switch (type) {
      ...
      case 'info': return 'bg-indigo-500 hover:bg-indigo-600 shadow-indigo-500/20';
      default: return 'bg-amber-500 hover:bg-amber-600 shadow-amber-500/20';
    }
  };
  ...
  return (
    ...
    >
      <div className="flex flex-col items-center text-center space-y-4 py-4">
        <div className={`w-16 h-16 rounded-[24px] bg-white/5 border border-white/10 flex items-center justify-center mb-2 animate-in zoom-in-50 duration-500`}>
          {getIcon()}
        </div>
