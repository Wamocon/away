...
export default function VacationList({
  ...
  if (loading) {
    return (
      <div className="text-xs dark:text-white/30 text-gray-400 py-4 text-center">
        Lade Anträge...
      </div>
    );
  }
    ...
    <div className="space-y-2">
      {requests.map((r) => {
        ...
        return (
          <div
            key={r.id}
            className={`rounded-lg border p-3 flex items-start justify-between gap-3 ${cfg.bg} ${cfg.border}`}
          >
            <div className="flex items-start gap-2.5 min-w-0">
              ...
              <div className="min-w-0">
                ...
                <div className={`text-[10px] font-semibold mt-1 ${cfg.color}`}>
                  {cfg.label}
                </div>
              </div>
            </div>
