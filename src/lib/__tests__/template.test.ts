  ...
  describe("getTemplatesForOrg", () => {
    it("returns templates for the org", async () => {
      mockDb.eq.mockResolvedValueOnce({
        data: [{ id: 1, file_name: "template.docx" }],
        error: null,
      });
    ...
    it("throws on error", async () => {
      ...
      await expect(getTemplatesForOrg("org-1")).rejects.toEqual({
        message: "db error",
      });
    });
  });
  ...
  describe("uploadTemplate", () => {
    it("uploads file and inserts record when it does not exist", async () => {
      ...
      mockStorage.upload.mockResolvedValueOnce({
        data: { path: "org-1/new-template.docx" },
        error: null,
      });
      ...
      expect(mockDb.insert).toHaveBeenCalledWith([
        { organization_id: "org-1", file_name: "new-template.docx" },
      ]);
    });
    ...
    it("throws on storage upload error", async () => {
      ...
      mockStorage.upload.mockResolvedValueOnce({
        data: null,
        error: { message: "storage fail" },
      });

      await expect(uploadTemplate("org-1", file)).rejects.toEqual({
  ...
  describe("getTemplateUrl", () => {
    it("returns public URL", () => {
      ...
      mockStorage.getPublicUrl.mockReturnValue({
        data: { publicUrl: "https://cdn.example.com/t.docx" },
      });
