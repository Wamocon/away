...
describe("admin lib - organization settings", () => {
  ...
  describe("updateOrganizationSettings", () => {
    it("should fetch and merge settings before updating", async () => {
      ...
      const result = await updateOrganizationSettings("org-123", {
        new: "setting",
      });
    ...
    it("should throw error if update fails", async () => {
      ...
      await expect(updateOrganizationSettings("org-123", {})).rejects.toThrow(
        "Update failed",
      );
    });
  });
  ...
  describe("getOrganizationSettings", () => {
    ...
    it("should warn but continue if initial settings fetch fails in updateOrganizationSettings", async () => {
      ...
      mockSupabase.single.mockResolvedValueOnce({
        data: null,
        error: new Error("settings column missing"),
      });
      // Update succeeds
      mockResult.error = null;
