...
describe('legal', () => {
  ...
  describe('saveLegalConsent', () => {
    it('inserts consent records without error', async () => {
      ...
      await expect(saveLegalConsent('user-1', ['agb', 'privacy'])).resolves.not.toThrow();
    ...
    it('inserts with custom version', async () => {
      ...
      await expect(saveLegalConsent('user-1', ['dsgvo'], '2.0')).resolves.not.toThrow();
    });
    ...
    it('throws if insert returns error', async () => {
      ...
      await expect(saveLegalConsent('user-1', ['agb'])).rejects.toEqual({ message: 'DB error' });
    });
  });
  ...
  describe('checkUserConsent', () => {
    ...
    it('returns data array on success', async () => {
      const chain = {
        select: vi.fn().mockReturnThis(),
        eq: vi.fn().mockResolvedValue({ data: [{ consent_type: 'agb' }], error: null }),
      };
      mockFrom.mockReturnValue(chain);
  ...
  describe('getOrganizationConsents', () => {
    it('returns users and consents on success', async () => {
      const usersChain = {
        select: vi.fn().mockReturnThis(),
        eq: vi.fn().mockResolvedValue({ data: [{ user_id: 'u1', role: 'admin' }], error: null }),
      };
      const consentsChain = {
        select: vi.fn().mockReturnThis(),
        in: vi.fn().mockResolvedValue({ data: [{ user_id: 'u1', consent_type: 'agb' }], error: null }),
      };
    ...
    it('throws on users query error', async () => {
      ...
      await expect(getOrganizationConsents('org-1')).rejects.toEqual({ message: 'fail' });
    });
    ...
    it('throws on consents query error', async () => {
      ...
      const consentsChain = {
        select: vi.fn().mockReturnThis(),
        in: vi.fn().mockResolvedValue({ data: null, error: { message: 'consents fail' } }),
      };
      ...
      await expect(getOrganizationConsents('org-1')).rejects.toEqual({ message: 'consents fail' });
    });
  });
});
