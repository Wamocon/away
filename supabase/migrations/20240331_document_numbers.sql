...
CREATE POLICY "View_Used_Doc_IDs" ON document_numbers
FOR SELECT USING (EXISTS (
    SELECT 1 FROM user_roles 
    WHERE organization_id = document_numbers.organization_id 
    AND user_id = auth.uid()
));

-- Any authenticated user can register their own document ID
CREATE POLICY "Insert_Doc_ID" ON document_numbers
...
GRANT ALL ON document_numbers TO authenticated, service_role;
