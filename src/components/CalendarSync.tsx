...
export default function CalendarSync({
  ...
  const handleConnect = async () => {
    if (!userId || userId.length < 32 || !orgId || orgId.length < 32) {
      setError(
        "Sitzungs- oder Organisationsdaten fehlen. Bitte laden Sie die Seite neu.",
      );
      return;
    }
    if (!email.trim()) {
    ...
    if (!token.trim()) {
      setError("Bitte Token / App-Passwort eingeben");
      return;
    }
    setError("");
    setSyncing(true);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      />
      <div
        className="relative w-full max-w-md card-glass p-8 animate-fade-in overflow-hidden"
        style={{ zIndex: 51 }}
      >
        ...
        <div className="flex items-center justify-between mb-8 relative">
          ...
          <button
            onClick={onClose}
            className="p-2 rounded-xl hover:bg-[var(--bg-elevated)] transition-colors text-[var(--text-muted)] hover:text-[var(--text-base)]"
          >
            <X size={18} />
          </button>
        </div>
        ...
        <div className="flex items-center gap-2 mb-8 px-2 relative">
          {(["choose", "connect", "select", "done"] as Step[]).map((s, i) => (
            <div key={s} className="flex items-center gap-2 flex-1">
              <div
                className={`wizard-step-dot !w-7 !h-7 !text-[10px] ${
                  step === s
                    ? "active"
                    : ["choose", "connect", "select", "done"].indexOf(step) > i
                      ? "done"
                      : "inactive"
                }`}
              >
              ...
              {i < 3 && (
                <div
                  className={`flex-1 h-[2px] rounded-full transition-all duration-500 ${["choose", "connect", "select", "done"].indexOf(step) > i ? "bg-[var(--success)] shadow-[0_0_8px_rgba(34,197,94,0.3)]" : "bg-[var(--border-subtle)]"}`}
                />
              )}
            </div>
          ))}
        ...
        {step === "choose" && (
          <div className="space-y-4 animate-in fade-in slide-in-from-bottom-4">
            <p className="text-xs font-bold uppercase tracking-widest opacity-50 mb-4 ml-1">
              Anbieter wählen
            </p>
            <button
            ...
            >
              <div className="w-12 h-12 rounded-xl bg-[#0078d4]/10 flex items-center justify-center group-hover:scale-110 transition-transform">
                <svg width="24" height="24" viewBox="0 0 24 24" fill="none">
                  <path
                    d="M2.5 12L12 2L21.5 12L12 22L2.5 12Z"
                    fill="#0078d4"
                    fillOpacity="0.1"
                  />
            ...
            <div className="p-4 rounded-2xl mt-6 flex items-start gap-3 bg-[var(--primary-light)] border border-[var(--primary-glow)]">
              ...
              <p
                ...
                style={{ color: "var(--text-base)" }}
              >
                Die Verbindung erfolgt über einen{" "}
                <strong>sicheren Zugriffsschlüssel</strong>. Diesen kannst du in
                deinen Account-Einstellungen unter "Sicherheit" generieren.
              </p>
            </div>
        ...
        {showGuide && (
          <div
            className="absolute inset-0 z-10 rounded-2xl overflow-y-auto p-8 animate-in fade-in slide-in-from-bottom-2"
            style={{
          ...
          >
              ...
              <ol className="space-y-4">
                {[
                  ...
                  },
                  {
                    n: 2,
                    title: "Anmelden",
                    desc: 'Klicke oben rechts auf "Sign in to Graph Explorer" und melde dich mit deinem Microsoft-Konto an.',
                  },
                  {
                    n: 3,
                    title: "Berechtigungen erteilen",
                    desc: 'Klicke auf "Modify Permissions" und aktiviere: Calendars.Read, Calendars.ReadWrite, Mail.Send',
                  },
                  {
                    n: 4,
                    title: "Access Token kopieren",
                    desc: 'Klicke auf den Tab "Access token" – dort siehst du einen langen Schlüssel. Diesen vollständig kopieren.',
                  },
                  {
                    n: 5,
                    title: "Hier einfügen",
                    desc: 'Zurück in AWAY: "Outlook" wählen, deine E-Mail + den kopierten Token einfügen und auf Verbinden klicken.',
                  },
                ].map((step) => (
                  <li key={step.n} className="flex gap-3">
                    <div className="shrink-0 w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-black text-white bg-[#0078d4]">
                      {step.n}
                    </div>
                    <div>
                      <p
                ...
                >
                  ⏱ Der Access Token ist <strong>60 Minuten</strong> gültig.
                  Danach muss ein neuer Token generiert werden.
                </li>
              </ol>
            ) : (
              <ol className="space-y-4">
                {[
                  ...
                  {
                    n: 2,
                    title: "Calendar API auswählen",
                    desc: 'Links unter Schritt 1: "Google Calendar API v3" aufklappen → "https://www.googleapis.com/auth/calendar.readonly" anklicken → "Authorize APIs"',
                  },
                  {
                    n: 3,
                    title: "Google-Konto auswählen",
                    desc: "Wähle dein Google-Konto und erteile die angefragten Berechtigungen.",
                  },
                  {
                    n: 4,
                    title: "Token tauschen",
                    desc: 'Klicke in Schritt 2 auf "Exchange authorization code for tokens". Du siehst dann das "Access token" Feld.',
                  },
                  {
                    n: 5,
                    title: "Token kopieren + einfügen",
                    desc: 'Das "Access token" vollständig kopieren. Zurück in AWAY: "Google" wählen, E-Mail + Token einfügen und verbinden.',
                  },
                ].map((step) => (
                  <li key={step.n} className="flex gap-3">
                    <div className="shrink-0 w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-black text-white bg-[#4285F4]">
                      {step.n}
                    </div>
                    <div>
                      <p
                ...
                >
                  ⏱ Der Access Token ist <strong>60 Minuten</strong> gültig. Für
                  dauerhaften Sync empfehlen wir die OAuth-App-Einrichtung.
                </li>
              </ol>
            )}
            ...
            <button
              onClick={() => {
                setShowGuide(null);
                handleChooseProvider(showGuide);
              }}
        ...
        {step === "connect" && (
          <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4">
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-1.5">
                ...
                <input
                  ...
                  onKeyDown={(e) =>
                    e.key === "Enter" && !syncing && handleConnect()
                  }
                ...
                <div className="mt-2 p-3 rounded-xl bg-[var(--bg-elevated)] border border-[var(--border)]">
                  ...
                  {provider === "outlook" ? (
                    ...
                    >
                      ...
                      → Anmelden → „Access Token" kopieren. Token ist 60 Min.
                      gültig.
                    </p>
                  ) : (
                    <p
                    ...
                    >
                      ...
                      → calendar.readonly wählen → Token generieren und
                      kopieren.
                    </p>
                  )}
                </div>
            ...
            {error && (
              <div
                className="text-[11px] font-bold p-3 rounded-xl animate-in shake-1"
                style={{
            ...
            <div className="flex gap-3 pt-2">
              <button
                ...
                className="flex-1 py-3 px-4 rounded-xl font-black text-[10px] uppercase tracking-widest border-2 border-[var(--border)] hover:bg-white/5 transition-all"
              >
                Zurück
              </button>
              <button
                ...
                disabled={syncing}
                className="flex-[2] py-3 px-4 rounded-xl bg-[var(--primary)] text-white font-black text-[10px] uppercase tracking-widest shadow-lg shadow-indigo-500/20 flex items-center justify-center gap-2 hover:scale-[1.02] active:scale-[0.98] transition-all disabled:opacity-50"
              >
                {syncing ? (
          ...
          <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4">
            <div className="flex items-center gap-3 p-4 rounded-2xl bg-emerald-500/5 border border-emerald-500/20">
              ...
              <div>
                ...
                <p className="text-[10px] opacity-60 text-emerald-600/80">
                  {events.length} Termine geladen
                </p>
              </div>
            </div>
            ...
            <div className="space-y-3">
              ...
              >
                <div
                  className={`w-5 h-5 rounded border flex items-center justify-center transition-all ${importAll ? "bg-[var(--primary)] border-[var(--primary)]" : "border-[var(--text-subtle)]"}`}
                >
                  {importAll && (
              ...
              {!importAll && (
                <div className="space-y-2 max-h-60 overflow-y-auto pr-1 custom-scrollbar">
                  {events.map((ev) => (
                    ...
                    >
                      <div
                        className={`w-4 h-4 rounded border flex items-center justify-center shrink-0 ${ev.selected ? "bg-[var(--primary)] border-[var(--primary)]" : "border-[var(--text-subtle)]"}`}
                      >
                        {ev.selected && (
            ...
            {error && (
              <div
                className="text-[11px] font-bold p-3 rounded-xl"
                style={{
                  background: "var(--danger-light)",
                  color: "var(--danger)",
                }}
            ...
            <div className="flex gap-3">
              ...
              <button
                ...
                disabled={syncing}
                className="flex-[2] py-3 px-4 rounded-xl bg-[var(--primary)] text-white font-black text-[10px] uppercase tracking-widest shadow-lg shadow-indigo-500/20 flex items-center justify-center gap-2 hover:scale-[1.02] transition-all"
              >
                {syncing ? (
                  <RefreshCw size={14} className="animate-spin" />
                ) : (
        ...
        {step === "done" && (
          <div className="text-center py-10 animate-in zoom-in-95 duration-500">
            ...
            <p className="text-sm font-medium opacity-60 mb-10 px-4">
              Deine Termine wurden erfolgreich importiert und stehen nun im
              Kalender zur Verfügung.
            </p>
            <button
              onClick={onClose}
              className="w-full py-4 rounded-2xl bg-[var(--primary)] text-white font-black text-xs uppercase tracking-widest shadow-xl shadow-indigo-500/20 hover:scale-[1.02] transition-all"
            >
              Kalender ansehen
            </button>
          </div>
