...
export async function toggleUserRoleAction(newRole: string) {
  try {
    ...
    if (orgError) {
      ...
      throw new Error(
        `Konnte Organisationen des Nutzers nicht laden: ${orgError.message}`,
      );
    }
    ...
    if (orgIds.length === 0) {
      console.log(
        `[DEBUG] Nutzer ${userId} hat keine Org-Verknüpfung. Suche System-Standard...`,
      );
    ...
    if (orgIds.length === 0) {
      throw new Error(
        "Keine Organisation im System gefunden. Bitte erstelle erst eine Organisation im Admin-Bereich (als Admin).",
      );
    }

    console.log(
      `[DEBUG] Nutzer ${userId} wechselt zu Rolle "${newRole}" in ${orgIds.length} Orgs`,
    );
