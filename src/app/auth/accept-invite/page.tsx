  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(() => {
      if (mounted && checkingSessionRef.current) {
        ...
        setError(
          "Keine aktive Einladung gefunden. Bitte stelle sicher, dass du den Link aus der E-Mail geklickt hast.",
        );
      }
    }, 5000);
    ...
    const {
      ...
      console.log(
        `Auth Event in AcceptInvite: ${event}`,
    ...
    const checkSessionManually = async () => {
      ...
      if (typeof window !== "undefined" && window.location.hash) {
        console.log(
          "AcceptInvite: Hash found in URL, attempting manual extraction...",
        );
        ...
        if (accessToken && refreshToken) {
          ...
          if (setSessionError) {
            console.error(
              "AcceptInvite: Error manually setting session:",
              setSessionError,
            );
      ...
      >
        ...
        <div>
          ...
          <p className="text-white/50 text-sm leading-relaxed">
            Du wurdest zu Away eingeladen. Setze jetzt dein Passwort, um dein
            Konto zu aktivieren.
          </p>
        </div>
