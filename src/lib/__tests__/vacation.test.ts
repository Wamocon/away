...
describe('vacation lib', () => {
  ...
  describe('createVacationRequest', () => {
    it('creates a request and returns data', async () => {
      ...
      const res = await createVacationRequest({
        ...
        from: '2024-05-01', to: '2024-05-05', reason: 'Urlaub',
      ...
      expect(mockSupabase.insert).toHaveBeenCalledWith([{
        ...
        from: '2024-05-01', to: '2024-05-05',
        reason: 'Urlaub', status: 'pending', template_fields: { guest: 'yes' }
      }]);
    });
    ...
    it('uses empty object when no template_fields', async () => {
      ...
      await createVacationRequest({
        userId: 'u2', organizationId: 'o1', from: '2026-06-01', to: '2026-06-03', reason: 'x',
      });
      expect(mockSupabase.insert).toHaveBeenCalledWith([expect.objectContaining({ template_fields: {} })]);
    });

    it('throws on DB error', async () => {
      mockSupabase.single.mockResolvedValueOnce({ data: null, error: { message: 'insert error' } });
      await expect(createVacationRequest({
        userId: 'u', organizationId: 'o', from: '2026-06-01', to: '2026-06-05', reason: 'x',
  ...
  describe('getVacationRequestsForOrg', () => {
    ...
    it('throws on error', async () => {
      mockSupabase.order.mockResolvedValueOnce({ data: null, error: { message: 'fail' } });
      await expect(getVacationRequestsForOrg('org-1')).rejects.toEqual({ message: 'fail' });
    });
  });

  describe('getMyVacationRequests', () => {
    it('returns requests for a user', async () => {
      mockSupabase.order.mockResolvedValueOnce({ data: [{ id: 'r1', user_id: 'u1' }], error: null });
    ...
    it('throws on error', async () => {
      mockSupabase.order.mockResolvedValueOnce({ data: null, error: { message: 'err' } });
      await expect(getMyVacationRequests('u1')).rejects.toEqual({ message: 'err' });
    });
  });
  ...
  describe('updateVacationStatus', () => {
    ...
    it('throws on error', async () => {
      mockSupabase.single.mockResolvedValueOnce({ data: null, error: { message: 'upd err' } });
      await expect(updateVacationStatus('req-1', 'rejected')).rejects.toEqual({ message: 'upd err' });
    });
  });
});
