  ...
  const handleOAuth = async (provider: 'microsoft' | 'google') => {
    ...
    try {
      ...
      const { error } = await supabase.auth.signInWithOAuth({
        provider: supabaseProvider as Parameters<ReturnType<typeof createClient>['auth']['signInWithOAuth']>[0]['provider'],
        options: {
      ...
      setError(`${provider === 'microsoft' ? 'Microsoft' : 'Google'} Login noch nicht konfiguriert. Credentials in Supabase-Einstellungen eintragen.`);
    } finally {
      ...
      >
        ...
        <div>
          <h2 className="text-3xl font-black text-white leading-tight mb-4">
            Urlaub im Blick –<br/>für dein ganzes Team
          </h2>
          <p className="text-white/50 text-sm leading-relaxed">
            Anträge stellen, genehmigen und den Überblick behalten. Alles an einem Ort.
          </p>
        </div>
      ...
      <div className="flex-1 flex items-center justify-center p-6 lg:p-12 overflow-y-auto">
        <div className="w-full max-w-sm py-12">
          ...
          <p className="text-sm mb-6" style={{ color: 'var(--text-muted)' }}>
            {isSignUp ? 'Erstelle dein Away-Konto' : 'Melde dich bei deinem Away-Konto an'}
          </p>
