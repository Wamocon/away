...
describe('holidays', () => {
  ...
  describe('calculateVacationDays', () => {
    it('returns 0 if from > to', () => {
      expect(calculateVacationDays(new Date(2024, 5, 10), new Date(2024, 5, 1), 'BY')).toBe(0);
    });

    it('excludes weekends', () => {
      // Monday to Friday = 5 work days
      const days = calculateVacationDays(new Date(2024, 5, 3), new Date(2024, 5, 7), 'BY');
      expect(days).toBe(5);
    });

    it('excludes public holidays', () => {
      // Tag der Deutschen Einheit (Oct 3, 2024) is a Thursday
      const days = calculateVacationDays(new Date(2024, 9, 3), new Date(2024, 9, 3), 'BY');
      expect(days).toBe(0);
    });

    it('handles same-day request (working day)', () => {
      // June 5, 2024 is a Wednesday
      const days = calculateVacationDays(new Date(2024, 5, 5), new Date(2024, 5, 5), 'BY');
      expect(days).toBe(1);
    });

    it('handles multi-year ranges', () => {
      const days = calculateVacationDays(new Date(2024, 11, 30), new Date(2025, 0, 2), 'BY');
