...
describe("Administration Actions", () => {
  ...
  describe("getOrgMembersWithEmails", () => {
    ...
    it("should handle successful member fetching", async () => {
      ...
      vi.mocked(getOrgMembersWithEmails).mockResolvedValue({
        data: mockMembers,
      } as any);
      ...
      expect(vi.mocked(getOrgMembersWithEmails)).toHaveBeenCalledWith(
        "org-123",
      );
    });
  ...
  describe("inviteUserToOrg", () => {
    ...
    it("should handle successful invitation", async () => {
      ...
      expect(vi.mocked(inviteUserToOrg)).toHaveBeenCalledWith(
        "new@test.com",
        "org-123",
        "employee",
        "http://localhost:3000",
    ...
    it("should handle invitation errors", async () => {
      vi.mocked(inviteUserToOrg).mockResolvedValue({
