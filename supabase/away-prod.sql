...
CREATE TABLE IF NOT EXISTS "away-prod".legal_consents (
  id           UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id      UUID NOT NULL,
...
ALTER TABLE "away-prod".legal_consents     ENABLE ROW LEVEL SECURITY;
...
CREATE POLICY "Settings_Admin_Read" ON "away-prod".user_settings FOR SELECT USING (public.is_admin_in_org('away-prod', organization_id));

-- vacation_requests
CREATE POLICY "Vac_Self"            ON "away-prod".vacation_requests FOR ALL    USING (auth.uid() = user_id);
CREATE POLICY "Vac_View_Approver"   ON "away-prod".vacation_requests FOR SELECT USING (public.is_approver_in_org('away-prod', organization_id));
CREATE POLICY "Vac_Update_Approver" ON "away-prod".vacation_requests FOR UPDATE USING (public.is_approver_in_org('away-prod', organization_id));

-- calendar_events
CREATE POLICY "Events_Self" ON "away-prod".calendar_events FOR ALL USING (auth.uid() = user_id);

-- document_templates
CREATE POLICY "Template_View"   ON "away-prod".document_templates FOR SELECT USING (EXISTS (SELECT 1 FROM "away-prod".user_roles WHERE organization_id = "away-prod".document_templates.organization_id AND user_id = auth.uid()));
CREATE POLICY "Template_Manage" ON "away-prod".document_templates FOR ALL    USING (public.is_admin_in_org('away-prod', organization_id));

-- document_numbers
CREATE POLICY "DocNum_View"     ON "away-prod".document_numbers FOR SELECT USING (EXISTS (SELECT 1 FROM "away-prod".user_roles WHERE organization_id = "away-prod".document_numbers.organization_id AND user_id = auth.uid()));
CREATE POLICY "DocNum_Register" ON "away-prod".document_numbers FOR INSERT WITH CHECK (auth.uid() = user_id);

-- legal_consents
CREATE POLICY "Consents_View_Own"   ON "away-prod".legal_consents FOR SELECT USING (auth.uid() = user_id);
CREATE POLICY "Consents_View_Admin" ON "away-prod".legal_consents FOR SELECT USING (EXISTS (SELECT 1 FROM "away-prod".user_roles WHERE user_id = auth.uid() AND role = 'admin'));
CREATE POLICY "Consents_Insert_Own" ON "away-prod".legal_consents FOR INSERT WITH CHECK (auth.uid() = user_id);

-- ── Indizes ────────────────────────────────────────────────

CREATE INDEX IF NOT EXISTS idx_vacation_requests_org_status  ON "away-prod".vacation_requests (organization_id, status);
CREATE INDEX IF NOT EXISTS idx_vacation_requests_org_created ON "away-prod".vacation_requests (organization_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_vacation_requests_user_org    ON "away-prod".vacation_requests (user_id, organization_id);
CREATE INDEX IF NOT EXISTS idx_document_numbers_org          ON "away-prod".document_numbers  (organization_id);
CREATE INDEX IF NOT EXISTS idx_document_numbers_user_org     ON "away-prod".document_numbers  (organization_id, user_id, created_at DESC);

-- ── Trigger: Standardwerte für user_settings ───────────────

DROP TRIGGER IF EXISTS trg_default_user_settings ON "away-prod".user_settings;
CREATE TRIGGER trg_default_user_settings
  BEFORE INSERT ON "away-prod".user_settings
  FOR EACH ROW EXECUTE FUNCTION public.set_default_user_settings();

-- ── Kompatibilitäts-View ───────────────────────────────────

CREATE OR REPLACE VIEW "away-prod".organization_members AS
