...
describe('organization lib', () => {
  ...
  describe('getOrganizationsForUser', () => {
    ...
    it('maps organization objects', async () => {
      mockSupabase.order.mockResolvedValueOnce({
        data: [{ organizations: { id: 'org-1', name: 'Acme' } }], error: null,
      });
      const res = await getOrganizationsForUser('user-1');
    ...
    it('flattens array organizations field', async () => {
      mockSupabase.order.mockResolvedValueOnce({
        data: [{ organizations: [{ id: 'o1', name: 'A' }, { id: 'o2', name: 'B' }] }], error: null,
      });
      const res = await getOrganizationsForUser('user-1');
    ...
    it('throws on error', async () => {
      ...
      await expect(getOrganizationsForUser('u')).rejects.toEqual({ message: 'fail' });
    });
  });
  ...
  describe('getCurrentOrganization', () => {
    ...
    it('throws on error', async () => {
      ...
      await expect(getCurrentOrganization('x')).rejects.toEqual({ message: 'not found' });
    });
  });
  ...
  describe('createOrganization', () => {
    it('calls RPC and returns new org', async () => {
      ...
      expect(mockSupabase.rpc).toHaveBeenCalledWith('create_new_organization', {
        org_name: 'New Org', creator_id: 'user-1',
      });
    });

    it('throws on RPC error', async () => {
      mockSupabase.rpc.mockResolvedValueOnce({ data: null, error: { message: 'rpc error' } });
      await expect(createOrganization('u', 'X')).rejects.toEqual({ message: 'rpc error' });
    });
  });
  ...
  describe('joinOrganization', () => {
    ...
    it('inserts role when not already a member', async () => {
      ...
      expect(mockSupabase.insert).toHaveBeenCalledWith([{
        user_id: 'user-1', organization_id: 'org-1', role: 'employee',
      }]);
    });

    it('throws on check error', async () => {
      mockSupabase.maybeSingle.mockResolvedValueOnce({ data: null, error: { message: 'check fail' } });
      await expect(joinOrganization('u', 'o')).rejects.toEqual({ message: 'check fail' });
    });

    it('throws on insert error', async () => {
      mockSupabase.maybeSingle.mockResolvedValueOnce({ data: null, error: null });
      mockSupabase.insert.mockResolvedValueOnce({ error: { message: 'join fail' } });
      await expect(joinOrganization('u', 'o')).rejects.toEqual({ message: 'join fail' });
    });
  });
  ...
  describe('updateOrganizationName', () => {
    ...
    it('throws on update error', async () => {
      ...
      await expect(updateOrganizationName('org-1', 'x')).rejects.toEqual({ message: 'upd fail' });
    });
  });
});
