...
type DateRangePreset = 'this_year' | 'last_30' | 'last_90' | 'last_year' | 'all';
...
function getDateRange(preset: DateRangePreset): { from: Date; to: Date } {
  const now = new Date();
  if (preset === 'this_year') return { from: startOfYear(now), to: endOfYear(now) };
...
export default function ReportsPage() {
  ...
  const totalDays = filtered.filter(r => r.status === 'approved')
    .reduce((sum, r) => sum + differenceInCalendarDays(parseISO(r.to), parseISO(r.from)) + 1, 0);
  const pending = filtered.filter(r => r.status === 'pending').length;
  const approved = filtered.filter(r => r.status === 'approved').length;
  const rejected = filtered.filter(r => r.status === 'rejected').length;

  const avgDays = approved > 0
    ? (filtered.filter(r => r.status === 'approved')
        .reduce((sum, r) => sum + differenceInCalendarDays(parseISO(r.to), parseISO(r.from)) + 1, 0) / approved).toFixed(1)
    : '0';
  ...
  const monthlyData = useMemo(() => {
    ...
    for (let i = 11; i >= 0; i--) {
      ...
      const key = `${getYear(d)}-${getMonth(d)}`;
      result.push({
        label: MONTH_NAMES[getMonth(d)] + (getYear(d) !== new Date().getFullYear() ? ` '${String(getYear(d)).slice(2)}` : ''),
        count: data[key] || 0,
      });
    }
    return result;
  ...
  const employeeStats = useMemo(() => {
    ...
    filtered.filter(r => r.status === 'approved').forEach(r => {
      const d = differenceInCalendarDays(parseISO(r.to), parseISO(r.from)) + 1;
    ...
    <div className="p-6 md:p-8 w-full max-w-6xl mx-auto animate-fade-in space-y-6">
            ...
            {showRangePicker && (
              ...
              >
                {(Object.keys(DATE_RANGE_LABELS) as DateRangePreset[]).map(key => (
                  <button
                    key={key}
                    onClick={() => { setDateRange(key); setShowRangePicker(false); }}
                    ...
                    style={{ color: dateRange === key ? 'var(--primary)' : 'var(--text-base)' }}
                  >
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
        {[
          ...
          { label: 'Ø Dauer (Tage)', value: avgDays, Icon: BarChart2, color: '#8b5cf6', border: 'border-purple-500' },
        ].map(({ label, value, Icon, color, border }) => (
          <div key={label} className={`card p-4 border-l-4 ${border}`}>
            <div className="flex items-center gap-2 mb-2 text-[9px] font-black uppercase tracking-widest" style={{ color: 'var(--text-muted)' }}>
              <Icon size={12} style={{ color }} />
              {label}
            </div>
            <div className="text-2xl font-black" style={{ color: 'var(--text-base)' }}>{value}</div>
          </div>
        ))}
      </div>
      ...
      <div className="card p-6">
        <h2 className="text-xs font-black uppercase tracking-widest mb-5 flex items-center gap-2" style={{ color: 'var(--text-subtle)' }}>
          <TrendingUp size={14} style={{ color: 'var(--primary)' }} /> Anträge pro Monat (letzte 12 Monate)
        </h2>
        <div className="flex items-end gap-1 h-36">
          {monthlyData.map((m, i) => (
            <div key={i} className="flex-1 flex flex-col items-center gap-1">
              <span className="text-[9px] font-bold" style={{ color: 'var(--text-subtle)' }}>{m.count > 0 ? m.count : ''}</span>
              <div
              ...
              />
              <span className="text-[8px] truncate w-full text-center" style={{ color: 'var(--text-subtle)' }}>{m.label}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        ...
        <div className="card p-6">
          ...
          {filtered.length === 0 ? (
            <p className="text-sm text-center py-8" style={{ color: 'var(--text-muted)' }}>Keine Anträge im gewählten Zeitraum</p>
          ) : (
                ...
                <div key={label}>
                  <div className="flex justify-between text-xs mb-1">
                    ...
                    <span className="font-bold" style={{ color }}>
                      {count} ({filtered.length > 0 ? Math.round((count / filtered.length) * 100) : 0}%)
                    </span>
                  </div>
                  <div className="h-2 rounded-full" style={{ background: 'var(--bg-elevated)' }}>
                    <div
                      className="h-full rounded-full transition-all duration-700"
                      style={{ width: `${filtered.length > 0 ? (count / filtered.length) * 100 : 0}%`, background: color }}
                    />
                  </div>
                </div>
              ))}
        ...
        <div className="card p-6">
          <h2 className="text-xs font-black uppercase tracking-widest mb-4 flex items-center gap-2" style={{ color: 'var(--text-subtle)' }}>
            <Users size={14} style={{ color: 'var(--primary)' }} /> Top 5 nach genehmigten Tagen
          </h2>
          {employeeStats.length === 0 ? (
            <p className="text-sm text-center py-8" style={{ color: 'var(--text-muted)' }}>Keine genehmigten Anträge im Zeitraum</p>
          ) : (
            <div className="space-y-2">
              {employeeStats.map((e, i) => (
                <div key={e.uid} className="flex items-center gap-3 py-2 border-b last:border-0" style={{ borderColor: 'var(--border)' }}>
                  <span className="text-xs font-black w-4 text-center" style={{ color: 'var(--text-muted)' }}>#{i + 1}</span>
                  <div className="flex-1">
                    <span className="text-xs font-mono" style={{ color: 'var(--text-base)' }}>{e.uid}</span>
                    <span className="text-[10px] ml-2" style={{ color: 'var(--text-muted)' }}>{e.count} Anträge</span>
                  </div>
                  <span className="text-xs font-bold" style={{ color: 'var(--primary)' }}>{e.days} Tage</span>
                </div>
              ))}
            </div>
          )}
        ...
        <div className="card p-6 lg:col-span-2">
          <h2 className="text-xs font-black uppercase tracking-widest mb-4 flex items-center gap-2" style={{ color: 'var(--text-subtle)' }}>
            <Calendar size={14} style={{ color: 'var(--primary)' }} /> Letzte Anträge im Zeitraum
          </h2>
          {filtered.length === 0 ? (
            <p className="text-sm text-center py-8" style={{ color: 'var(--text-muted)' }}>Keine Anträge im gewählten Zeitraum</p>
          ) : (
              ...
              <table className="w-full text-xs">
                <thead>
                  <tr className="border-b" style={{ borderColor: 'var(--border)' }}>
                    {['Von', 'Bis', 'Tage', 'Grund', 'Status', 'Eingereicht'].map(h => (
                      <th key={h} className="text-left pb-2 pr-4 font-black uppercase tracking-wider text-[9px]" style={{ color: 'var(--text-subtle)' }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                    ...
                    .map(r => {
                      ...
                      return (
                        <tr key={r.id} className="border-b last:border-0" style={{ borderColor: 'var(--border)' }}>
                          <td className="py-2 pr-4" style={{ color: 'var(--text-base)' }}>{format(parseISO(r.from), 'dd.MM.yyyy', { locale: de })}</td>
                          <td className="py-2 pr-4" style={{ color: 'var(--text-base)' }}>{format(parseISO(r.to), 'dd.MM.yyyy', { locale: de })}</td>
                          <td className="py-2 pr-4 font-bold" style={{ color: 'var(--text-base)' }}>{days}</td>
                          <td className="py-2 pr-4 truncate max-w-[120px]" style={{ color: 'var(--text-muted)' }}>{r.reason || '–'}</td>
                          <td className="py-2 pr-4">
                            <span className="px-2 py-0.5 rounded-full text-[10px] font-bold" style={{ background: `${statusColor}20`, color: statusColor }}>
                              {r.status === 'approved' ? 'Genehmigt' : r.status === 'rejected' ? 'Abgelehnt' : 'Ausstehend'}
                            </span>
                          </td>
                          <td className="py-2" style={{ color: 'var(--text-muted)' }}>{format(parseISO(r.created_at), 'dd.MM.yy HH:mm', { locale: de })}</td>
                        </tr>
                      );
                    })}
                </tbody>
