...
import { inviteUserToOrg, getOrgMembersWithEmails, getOrgApproversForNotification } from '../adminActions';
...
describe('getOrgMembersWithEmails', () => {
  ...
  it('returns members with emails when admin', async () => {
    ...
    // Roles list
    mockSupabase.order.mockResolvedValueOnce({ data: [{ user_id: 'u1', role: 'admin', created_at: '' }], error: null });
    // getUserById
    mockAdminClient.auth.admin.getUserById.mockResolvedValue({ data: { user: { email: 'admin@x.de' } } });
...
describe('getOrgApproversForNotification', () => {
  ...
  it('returns members with emails from user_settings', async () => {
    ...
    const getUserById = vi.fn().mockResolvedValue({ data: { user: { email: 'approver@org.de' } } });
    vi.mocked(createSupabaseClient).mockReturnValue({
      auth: { admin: { getUserById } },
      from: vi.fn().mockReturnValue({
        select: vi.fn().mockReturnValue({
          eq: vi.fn().mockReturnValue({
            in: vi.fn().mockResolvedValue({ data: [{ user_id: 'u1', role: 'admin' }], error: null }),
            eq: vi.fn().mockReturnValue({
  ...
  it('falls back to auth admin email when user_settings has no email', async () => {
    mockAdminClient.in.mockResolvedValueOnce({
      data: [{ user_id: 'u1', role: 'cio' }], error: null,
    });
    // user_settings → no email
    const mockMaybeSingle = vi.fn().mockResolvedValue({ data: null, error: null });
    const mockEq = vi.fn().mockReturnValue({ maybeSingle: mockMaybeSingle });
    mockAdminClient.from.mockReturnValue({ 
      select: vi.fn().mockReturnValue({ eq: vi.fn().mockReturnValue({ eq: mockEq }) }) 
    });
    ...
    vi.mocked(createSupabaseClient).mockReturnValue({
      ...
      from: vi.fn()
        .mockReturnValueOnce({ 
          select: vi.fn().mockReturnValue({
            eq: vi.fn().mockReturnValue({
              in: vi.fn().mockResolvedValue({ data: [{ user_id: 'u1', role: 'cio' }], error: null })
            })
          })
        })
        .mockReturnValue({ 
  ...
  it('returns empty array on unexpected error', async () => {
    vi.mocked(createSupabaseClient).mockImplementation(() => { throw new Error('fatal'); });
    const res = await getOrgApproversForNotification('org-1');
    expect(res).toEqual([]);
  });
});
