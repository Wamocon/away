...
CREATE POLICY "Calendar_View" ON "away-prod".calendar_events FOR SELECT USING (EXISTS (SELECT 1 FROM "away-prod".user_roles WHERE organization_id = "away-prod".calendar_events.organization_id AND user_id = auth.uid()));
CREATE POLICY "Calendar_Manage" ON "away-prod".calendar_events FOR ALL USING (auth.uid() = user_id);
