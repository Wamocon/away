...
describe('calendarSync lib', () => {
  ...
  describe('saveOAuthSettings', () => {
    ...
    it('throws if orgId is too short', async () => {
      await expect(saveOAuthSettings(VALID_ID, 'short', 'google', 'a@b.com', 'tok'))
        .rejects.toThrow('Organisations-ID fehlt oder ist ungültig.');
    });

    it('merges with existing settings and calls upsert', async () => {
      mockSupabase.maybeSingle.mockResolvedValueOnce({
        data: { settings: { some_key: 'existing_value' } }, error: null,
      });
      mockSupabase.upsert.mockResolvedValueOnce({ error: null });

      await saveOAuthSettings(VALID_ID, VALID_ID, 'outlook', 'test@test.com', 'tok123');
    ...
    it('calls upsert with no existing settings', async () => {
      ...
      await saveOAuthSettings(VALID_ID, VALID_ID, 'google', 'g@g.com', 'goog-tok');

      expect(mockSupabase.upsert).toHaveBeenCalledWith(
        expect.objectContaining({
          settings: expect.objectContaining({ google_email: 'g@g.com', google_token: 'goog-tok' }),
        }),
        expect.any(Object),
      );
    });
    ...
    it('throws on upsert error', async () => {
      mockSupabase.maybeSingle.mockResolvedValueOnce({ data: null, error: null });
      mockSupabase.upsert.mockResolvedValueOnce({ error: { message: 'upsert fail' } });

      await expect(saveOAuthSettings(VALID_ID, VALID_ID, 'outlook', 'a@b.com', 'tok'))
  ...
  describe('importCalendarEvents', () => {
    it('upserts calendar events', async () => {
      ...
      const events: CalendarEventInput[] = [{
        userId: 'u1', orgId: 'o1', provider: 'outlook', externalId: 'e1',
        title: 'Meeting', startDate: '2025-06-01', endDate: '2025-06-01',
      }];
    ...
    it('throws on upsert error', async () => {
      ...
      await expect(importCalendarEvents([])).rejects.toEqual({ message: 'err' });
    });
  });
  ...
  describe('getSyncedEvents', () => {
    ...
    it('throws on error', async () => {
      mockSupabase.order.mockResolvedValueOnce({ data: null, error: { message: 'db err' } });
      await expect(getSyncedEvents('org-1')).rejects.toEqual({ message: 'db err' });
    });
  });
  ...
  describe('getMicrosoftOAuthUrl', () => {
    it('returns a URL containing the client_id', () => {
      const url = getMicrosoftOAuthUrl('my-client-id', 'https://myapp.com/callback');
  ...
  describe('fetchExternalEvents', () => {
    it('throws for short/empty token', async () => {
      await expect(fetchExternalEvents('outlook', '')).rejects.toThrow('Ungültiger Token');
      await expect(fetchExternalEvents('google', 'short')).rejects.toThrow('Ungültiger Token');
    });

    it('throws 401 for outlook on expired token', async () => {
      vi.stubGlobal('fetch', vi.fn().mockResolvedValue({ status: 401, ok: false, text: async () => 'Unauthorized' }));
      await expect(fetchExternalEvents('outlook', 'a'.repeat(20))).rejects.toThrow('Microsoft-Token abgelaufen');
      vi.unstubAllGlobals();
    });

    it('throws generic error for non-ok outlook response', async () => {
      vi.stubGlobal('fetch', vi.fn().mockResolvedValue({ status: 500, ok: false, text: async () => 'Server Error' }));
      await expect(fetchExternalEvents('outlook', 'a'.repeat(20))).rejects.toThrow('Microsoft Graph Fehler (500)');
      vi.unstubAllGlobals();
    });
    ...
    it('returns mapped outlook events on success', async () => {
      vi.stubGlobal('fetch', vi.fn().mockResolvedValue({
        status: 200, ok: true,
        json: async () => ({
          value: [{ id: 'ev1', subject: 'All Hands', start: { dateTime: '2025-06-01T10:00:00' }, end: { dateTime: '2025-06-01T11:00:00' }, isAllDay: false }],
        }),
      }));
    ...
    it('uses fallback title when outlook subject is missing', async () => {
      vi.stubGlobal('fetch', vi.fn().mockResolvedValue({
        status: 200, ok: true,
        json: async () => ({
          value: [{ id: 'ev1', start: { dateTime: '2025-06-01T10:00:00' }, end: { dateTime: '2025-06-01T11:00:00' } }],
        }),
      }));
      const events = await fetchExternalEvents('outlook', 'a'.repeat(20));
    ...
    it('throws 401 for google on expired token', async () => {
      vi.stubGlobal('fetch', vi.fn().mockResolvedValue({ status: 401, ok: false, text: async () => 'Unauthorized' }));
      await expect(fetchExternalEvents('google', 'a'.repeat(20))).rejects.toThrow('Google-Token abgelaufen');
      vi.unstubAllGlobals();
    });

    it('throws generic error for non-ok google response', async () => {
      vi.stubGlobal('fetch', vi.fn().mockResolvedValue({ status: 403, ok: false, text: async () => 'Forbidden' }));
      await expect(fetchExternalEvents('google', 'a'.repeat(20))).rejects.toThrow('Google Calendar Fehler (403)');
      vi.unstubAllGlobals();
    });
    ...
    it('returns mapped google events', async () => {
      vi.stubGlobal('fetch', vi.fn().mockResolvedValue({
        status: 200, ok: true,
        json: async () => ({
          items: [{ id: 'g1', summary: 'Google Meet', start: { dateTime: '2025-06-02T09:00:00' }, end: { dateTime: '2025-06-02T10:00:00' } }],
        }),
      }));
    ...
    it('sets allDay=true for google events with date-only start', async () => {
      vi.stubGlobal('fetch', vi.fn().mockResolvedValue({
        status: 200, ok: true,
        json: async () => ({
          items: [{ id: 'g2', summary: 'Holiday', start: { date: '2025-12-25' }, end: { date: '2025-12-26' } }],
        }),
      }));
      const events = await fetchExternalEvents('google', 'a'.repeat(20));
